...
# This is the Win32 compatible one, using Win32::SerialPort and
# Geo::IP::PurePerl instead
...
my $port = Win32::SerialPort->new($ser) || die "Unable to open serial device $ser\n";
$port->baudrate(9600);
$port->handshake("none");
$port->databits(8);
$port->stopbits(1);

my $gi = Geo::IP::PurePerl->open('./GeoIP.dat',GEOIP_MEMORY_CACHE);
